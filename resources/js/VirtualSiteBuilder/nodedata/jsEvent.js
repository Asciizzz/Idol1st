...
export class VsbJsEventData extends VsbNodeData {
    constructor({ event = "", code = "", name = "New JS Event", vsgraph = {} } = {}) {
        super({ type: VsbNodeType.JS_EVENT, name, vsgraph });
        this.event = event;
        this.code  = code;
    }
    ...
    static createFn({ node, graph, vsgraph } = {}) {
        ...
        element.style.outlineColor = VsbJsEventData._fileColor();
    ...
    static renderFn({ node, element, graph, vsgraph, cache, ctx }) {
        ...
        cache.meta.textContent  = data.event ?? "event";
